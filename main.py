...
def create_augmented_dataloader(args, dataset):
    ...
    # Select 5000 random examples from training set for transformation
    # Shuffle and select 5000 examples
    transformed_subset = original_train.shuffle(seed=42).select(range(5000))
    
    # Apply custom transformation
    transformed_data = transformed_subset.map(custom_transform, load_from_cache_file=False)
    
    # Concatenate original and transformed data
    augmented_dataset = concatenate_datasets([original_train, transformed_data])
